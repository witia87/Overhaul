...
{
    public class Investigate : VehicleCognitionState
    {
        ...
        public Investigate(Cognition<VehicleCognitionStateIds> parrentCognition, Vector3 targetLastSeenPosition,
            Vector3 targetLastSeenDirection)
            : base(parrentCognition, VehicleCognitionStateIds.Investigate)
        {
            ...
            if (MountedModules.IsHumanoidMovementControlMounted)
            {
                ...
                if (IsStuck())
                {
                    return new Unstuck(ParrentCognition,
                        -Core.gameObject.transform.forward, _targetLastSeenPosition, 1, this);
                }
                ManageMovement();
            }
