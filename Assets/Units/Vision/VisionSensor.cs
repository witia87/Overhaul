...
namespace Assets.Units.Modules.Coordinator.Vision
{
    public class VisionSensor : DiamondDetector, IVisionSensor
    {
        ...
        {
            _visionObserver = FindObjectOfType<VisionStore>().RegisterUnit(GetComponentInParent<Unit>());
            Length = VisionLenght;
        ...
        {
            if (other.tag == TagToDetect) CollidingGameObjects.Add(other.gameObject);
        }

        public void Update()
        {
            foreach (var visibleTarget in _visibleTargets) visibleTarget.IsVisible = false;
            foreach (var collidingObject in CollidingGameObjects)
            {
                    ...
                    && collidingObject.transform.root == hit.transform.root)
                {
                    ...
                    if (unit != null) UpdateOrAddTarget(unit);
        ...
        private void UpdateOrAddTarget(Unit unit)
        {
            for (var i = 0; i < _visibleTargets.Count; i++)
                if (_visibleTargets[i].BodyControl == unit)
                {
